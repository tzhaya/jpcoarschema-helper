...
3. **本ガイドの運用方針** — CONVENTIONS.md（例: `xml:lang` は原則付与）
...
7. 文章はこの時点から `japanese-tech-writing` の規範に従って書く。独立した推敲工程での書き直しに頼らない。
8. [decision-trees/README.md](../../../decision-trees/README.md) の一覧表（フローチャート一覧・進捗の正本）を更新。

## ステップ3: 照合・検証（省略しない）

完成物を両典拠に突き合わせる。チェックは検査の目的ごとに3群に分ける。

A群は公式説明ページの再取得を伴う。新しいプロパティを作るとき、公式定義や記入ルールに関わる改訂をするとき、`reference/*_rules.md` を変更するときに実施する。
表記、リンク、節参照だけを直す軽微な修正では、A群を省略し、変更した文と既存の `*_rules.md` の整合だけを確認する。

### A群: 公式資料との照合（公式ページの再取得を伴う）

- [ ] 公式ページを再取得し、**注意点欄**と**非推奨例欄**を本文と照合したか（説明欄だけを見ると落ちる。実例は Issue #26）
- [ ] 記入レベル・繰返・属性が `*_rules.md`（＝公式）と一致するか
- [ ] 必須性を表す語（必須・必ず・してはならない）を弱めずに転記したか
- [ ] ある要素の規定を**別の要素へ転用していない**か（例: タイトルのヨミ規定を作成者へ持ち込む）

### B群: 根拠と層の点検（本文の目視と grep）

- [ ] 公式要件として断定した記述に一次資料の根拠があるか（公式にない概念や区分を、公式の規定として書いていないか）
- [ ] 推論と運用方針を、そうと分かる書き方にしたか（CONVENTIONS.md 第5節「運用方針と解釈を示す表現」）
- [ ] 「多い」「多数」などの頻度の主張に出典があるか（なければ条件や手順の記述に置き換える）
- [ ] 適用する版と条件を示したか（スキーマ 2.0、対照表 ver.1.5 など）
- [ ] 要素名が実在するか（`jpcoar:version` のような存在しない要素を書いていないか）
- [ ] DOI要件の語（必須/条件付必須/任意/**推奨**）を取り違えていないか（特に JaLC の「任意」と「推奨」）
- [ ] Crossref の `xml:lang="en"` 必須・ORCID限定・書籍系の対象タイプを正しく反映したか
- [ ] スキーマ層・DOI登録層・本ガイドの運用方針が混ざっていないか
...
本文はステップ2の執筆時から `japanese-tech-writing` に従って書いているため、ステップ4は書き直しではなく、規範からの逸脱を点検する工程とする。

- 新規作成と大規模改訂では、`cognitive-rhythm-writing` を導入と注記に適用する。参照用の早見表、対応表、XML例には適用しない。
- 軽微な修正では、独立した推敲工程を省略し、変更した文が規範に沿っているかだけを確認する。
- 冒頭が、ページ構成の説明ではなく、入力時に生じる具体的な迷いや判断の差から始まっているか確認する。
...
点検で変更した文について、ステップ3の該当項目を再照合する。
変更がなければ再照合は不要とし、作業を完了する。
