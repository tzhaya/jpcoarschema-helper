...
| `departmental bulletin paper`、`other` | 紀要類に掲載された論文は `departmental bulletin paper`。ただし、公式の資源タイプ語彙別表では、紀要類の表紙や目次は `other` とする |
...
| `journal`、`other periodical` | 学術研究や最新動向を広める逐次刊行物そのものは `journal`。語彙名にかかわらず、`other periodical` の公式定義は「既存の語彙に該当しない「テキスト」資料」 |
...
| `conference object` | 1.0.2の公式別表では語彙名でしたが、2.0ではカテゴリ見出しとして残り、配下の現行語彙名は `conference output` です。URI `c_c94f` は同一です |
