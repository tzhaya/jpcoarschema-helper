...
`jpcoar:creator` はスキーマ上 **MA（条件に当てはまる場合は記載・0-N）** です。作成者がいる場合は、第一著者から順に記載します。作成者を記載する場合、作成者姓名 `jpcoar:creatorName` は DOI登録で **条件付必須**（作成者がある場合は必須）です。
...
`xml:lang` は、氏名・名称の言語を示す属性です。姓名（`creatorName` / `familyName` / `givenName`）では **JaLC DOI＝推奨 / Crossref DOI＝必須** です。本ガイドの運用方針としては、入力漏れと多言語混在を防ぐため、**原則としてすべての氏名に付与**します。
...
flowchart TD
    ...
    D5{所属はあるか?}
    D5 -- "はい" --> A5["jpcoar:affiliation > jpcoar:affiliationName を記載<br/>複数言語なら xml:lang 必須<br/>（可能なら所属機関識別子 ROR 等）"]
    D5 -- いいえ --> D6
    A5 --> D6
...
> **ポイント**: まず `jpcoar:creatorName` に氏名・名称を記載します（作成者がある場合は必須）。個人名は `familyName` ＋ `givenName` を補足します。Crossref DOI では姓名（`creatorName` / `familyName` / `givenName`）に `xml:lang` が必須で、作成者識別子は ORCID に限定されます。
...
| | | いいえ | #5 へ | ― | ― |
| #5 | 所属はあるか | はい | 所属機関名を記載（複数言語なら `xml:lang` 必須） | `jpcoar:affiliation` > `jpcoar:affiliationName` | 国立情報学研究所 |
| | | いいえ | #6 へ | ― | ― |
...
- **要素の必須度**: `jpcoar:creator` はスキーマ上 MA（条件に当てはまる場合は記載・0-N）です。作成者がいる場合は記載し、複数いる場合は繰り返し記載できます。作成者を記載する場合、`jpcoar:creatorName` は DOI登録で**条件付必須**です。
- **`xml:lang` の意味と扱い**: `xml:lang` は氏名・名称の言語を示す属性です。姓名は JaLC DOI＝推奨 / Crossref DOI＝必須です。本ガイドでは原則付与します。
- **記載順**: 複数の作成者がいる場合は **第一著者から順に** 記載します。
- **作成者と寄与者の区別**: 直接的に作成に関与した者を「作成者」、間接的に関与した者（`contributorType` の役割に該当する者など）は「寄与者」(#4) に記載します。学位論文の場合は作成者を必ず記載します。
- **creatorName と familyName/givenName の使い分け**:
  - すべての作成者について、まず `jpcoar:creatorName`（姓名・名称を1文字列）を記載します。個人名は「姓, 名」（姓・カンマ・スペース区切り）で記載します。
  - **個人名**で姓と名が判別できる場合は、加えて `familyName` ＋ `givenName` を補足すると検索・典拠連携に有利です（いずれも任意、あれば記載）。
...
- **別名**: 旧姓・筆名などは `jpcoar:creatorAlternative` に記載します（記述方法は `creatorName` に準じる）。
- **作成者識別子 `nameIdentifierScheme` の値**（統制語彙）:
  `e-Rad_Researcher` / `ORCID` / `ISNI` / `VIAF` / `AID` / `Ringgold` / `ROR` ほか。
  ※`NRID` / `kakenhi` / `GRID` は **非推奨**。ID は接頭辞を付けず ID のみを記載し、HTTP URI は `nameIdentifierURI` に記載します。
- **所属機関名**: 略称ではなく正式名称を、機関名まで（部局名は記載しない）記載します。所属機関識別子のスキームは `ISNI` / `Ringgold` / `ROR` ほか（`kakenhi` / `GRID` は非推奨）。
- **DOI登録先による差分**（[対照表](../reference/JPCOAR_JaLC_Crossref_requirements.md) より）:
  - **JaLC DOI**: 作成者姓名は **条件付必須**（作成者を記載する場合は必須）。姓名 `xml:lang` は推奨。識別子スキームは制限なし。
